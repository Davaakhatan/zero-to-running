...
APP_FRONTEND_LB=$(kubectl get svc app-frontend-service -n dev-env -o jsonpath='{.status.loadBalancer.ingress[0].hostname}' 2>/dev/null)
...
APP_FRONTEND_PORT=$(kubectl get svc app-frontend-service -n dev-env -o jsonpath='{.spec.ports[0].port}' 2>/dev/null || echo "3000")
...
if [ -n "$APP_FRONTEND_LB" ]; then
    echo "✅ App Frontend (Quote App) is ready!"
    echo "   URL: http://$APP_FRONTEND_LB:$APP_FRONTEND_PORT"
    echo ""
else
    echo "⏳ App Frontend LoadBalancer is still being provisioned..."
    echo "   Run this script again in a few minutes"
    echo ""
fi

if [ -n "$DASHBOARD_LB" ] && [ -n "$COLLABCANVA_LB" ] && [ -n "$APP_FRONTEND_LB" ]; then
    echo "🎉 All services are ready! Share these URLs:"
    echo ""
    echo "📋 Dashboard:"
    ...
    echo ""
    echo "📋 App Frontend (Quote App):"
    echo "   http://$APP_FRONTEND_LB:$APP_FRONTEND_PORT"
    echo ""
    echo "💡 These URLs are publicly accessible from anywhere!"
    echo "⚠️  Note: Port numbers are required in the URLs"
fi
